...
contract LiquidityPoolTest is Test {
    ...
    function testApprovals() public view {
        uint256 approvedAmount = gensToken.allowance(
            address(this),
            address(pool)
        );
        assertEq(
    ...
    function testRemoveLiquidity() public {
        ...
        uint liquidityTokens = pool.balanceOf(address(this));
        console.log("Liquidity Tokens Owned:", liquidityTokens);

        // Assuming the test wants to remove all liquidity:
        uint liquidityAmount = liquidityTokens; // Update this if not removing all

        console.log(
            "ETH Balance before removeLiquidity:",
        ...
        console.log(
            "ETH Balance after removeLiquidity:",
            address(this).balance
        );
        ...
        assertEq(
            gensToken.balanceOf(address(pool)),
            0,
            "The liquidity pool still has GENS tokens"
        );
        ...
        assertEq(
            ...
            0,
            "The liquidity pool still has Ether"
        );
        ...
        assertEq(
            pool.totalSupply(),
            0,
            "The liquidity pool still has liquidity tokens"
        );
    }
    ...
    function testGetReserves() public {
        ...
        (uint ethReserve, uint gensReserve) = pool.getReserves();
